...
// ============================================================
// STATION SHEET (Session Management) - ✅ تم التعديل هنا
// ============================================================
async function openStationSheet(stationId) {
    ...
    // ✅ التعديل هنا: إزالة الشرط `!isCountdown` لعرض الزرار في كل الأحوال
    body.innerHTML = `
        ...
        <div style="margin-top:16px;display:flex;flex-direction:column;gap:8px;">
            <!-- ✅ زر التحويل ظاهر دلوقتي في كل الحالات -->
...
function renderMenuQuickAdd() {
    ...
    let html = '';
    // تحديد أول تصنيف يبقى مفتوح
    const categoryNames = Object.keys(grouped);
    
    for (let i = 0; i < categoryNames.length; i++) {
        const category = categoryNames[i];
        const items = grouped[category];
        // التصنيف الأول بس هو اللي يبقى مفتوح افتراضيًا
        const isOpen = (i === 0);
        // حفظ الحالة
...
function renderSettings() {
    ...
    // تجميع الأصناف حسب التصنيف لعرضها في الإعدادات
    const groupedMenu = {};
    menuItems.forEach(item => {
        const category = normalizeMenuCategory(item.category);
...
async function handleSwitchMode(sessionId, newMode, stationId) {
    ...
    try {
        ...
        // ✅ التغيير الرئيسي: كسر سور المنع مع شبكة أمان للوقت الصفر
        if (activeSeg && activeSeg.timer_type === 'countdown') {
            ...
            // لو باقي وقت > 0، هنكمل التحويل عادي
        }
        ...
        // ✅ التغيير: حساب الوقت المتبقي للتنازلي
        if (activeSeg.timer_type === 'countdown' && activeSeg.duration_seconds) {
        ...
        // ✅ التغيير: المدة المتبقية للجزء الجديد (في التنازلي)
        ...
        // أضف السطرين دول:
        renderStationsGrid();   // عشان تحدّث الأجهزة في الصفحة الرئيسية
        renderDashboard();      // عشان تحدّث الإحصائيات لو لزم الأمر (اختياري لكن مفيد)
